...
const receipt: Record<string, unknown> = {
  ...
  scope: 'accepted_facts_to_analysis_only_P01_stubbed', createdAt: new Date().toISOString(),
...
try {
  ...
  const sourceText = '2025-06-10 LDL-C 4.2 mmol/L，参考范围 0-3.4 mmol/L，偏高。此文件仅用于软件测试。';
  const imported = await service.importFiles([{
    path: '/tmp/纯合成血脂报告.txt', bytes: Buffer.from(sourceText, 'utf8')
  }], personId);
  ...
  const extraction: ExtractionResult = {
    ...
    candidates: [{
      localKey: 'synthetic-ldl', originalName: 'LDL-C', standardNameCandidate: 'LDL-C',
      ...
      evidence: [{ sourceSpanId: spanId, quote: '2025-06-10 LDL-C 4.2 mmol/L' }], issues: []
    }]
  };
  ...
  if (facts.length !== 1 || facts[0]?.rawText !== '4.2') throw new Error('SYNTHETIC_FACTS_MISMATCH');
  ...
  const legacySnapshot = service.store.listSystemAnalysisSnapshots(personId, true)
    .find((snapshot) => snapshot.systemId === 'cardiovascular');
  ...
  receipt.comparisonComplete = (legacy.status === 'published' || legacy.status === 'rejected')
    && v3.status === 'published' && legacyCall === 2 && v3Call === 1
    && calls.every((call) => call.webSearches === 0);
