...
| 私有环境 stdio | PASS（未登录） | initialize/initialized/account-read/model-list 在私有空环境真实通过；`authenticated=false` |
| ESLint | PASS | 全工程 0 error |
| TypeScript 严格检查 | PASS | Node 与 Renderer 两套 tsconfig 均通过 |
| 自动测试 | PASS | 24 个文件、142 项测试，常规门禁现包含 Codex 0.145.0 Web Search 禁用/实时 `thread/start` 兼容性探针、500 文档/5 万指标容量基准、Mach-O/PE 目标识别、发行工件目标架构清理和冒烟工作区路径门禁；另覆盖 40 个纯合成来源包/1,000 个字段/20% 留出/18 个安全故障样例及分项计算；同时覆盖 DOCX 表格/嵌入图、旧 DOC 原件保留、组件缺失、固定哈希/版本、独立 profile、无 UNO listener、超时、PDF 输出和转换视图、SQLite 锁/模拟磁盘满、schema v5 迁移与恢复、恶意备份、恢复取消/故障恢复等 |
...
| 真实 Codex 登录/图像/结构化输出/取消 | NOT_RUN | 必须由用户在成品发起官方登录，再用纯虚构样例验证 |
...
| AT051–AT070 事实与派生主流程 | PASS（Mock/规则） | 两次独立事实处理、覆盖门禁、CAS 发布、安全复核；越界说明只阻断派生层，不回滚事实 |
...
- 真实 Codex 的认证、额度消耗、图像输入、结构化输出、取消与超时行为。
