...
| 自动测试 | PASS | 23 个文件、122 项测试，常规门禁现包含 500 文档/5 万指标容量基准、Mach-O/PE 目标识别、发行工件目标架构清理和冒烟工作区路径门禁；另覆盖 40 个纯合成来源包/1,000 个字段/20% 留出/18 个安全故障样例及分项计算；同时覆盖 DOCX 表格/嵌入图、旧 DOC 原件保留、组件缺失、固定哈希/版本、独立 profile、无 UNO listener、超时、PDF 输出和转换视图、SQLite 锁/模拟磁盘满、schema v5 迁移与恢复、恶意备份、恢复取消/故障恢复等 |
