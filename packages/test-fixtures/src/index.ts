...
export function createDemoSnapshot(now = new Date('2026-09-17T10:30:00+08:00')): DashboardSnapshot {
  ...
  return {
    ...
    reviews: [
      { id: 'review-1', personId: null, documentId: 'document-inbox-3', kind: 'person_conflict', severity: 'blocking', title: '确认门诊报告属于谁', description: '报告里的姓名与文件夹信息不能唯一匹配。确认之前不会把内容写入任何成员档案。', evidenceRefs: ['span-docx-heading'], candidateOptions: [], candidateDiffs: [], reportedName: null, resolutionStatus: 'open' }
    ],
    actions: [
