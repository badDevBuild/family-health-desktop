...
function InboxPage({ snapshot, onProcess, onImport, onDropFiles, onAssign, onIgnore, onOpenEvidence, onDirectories }: { snapshot: DashboardSnapshot; onProcess(documentIds?: string[]): void; onImport(): void; onDropFiles(files: File[], personId: string | null): void; onAssign(documentIds: string[], personId: string): Promise<void>; onIgnore(documentIds: string[]): Promise<void>; onOpenEvidence(evidence: Evidence): void; onDirectories(): void }) {
  ...
  const [visibleCount, setVisibleCount] = useState(50);
  const items = snapshot.inbox.filter((item) => (filter === 'all' || item.status === filter)
    && (personFilter === 'all' || (personFilter === 'unassigned' ? item.personId === null : item.personId === personFilter)));
  const visibleItems = items.slice(0, visibleCount);
  const selected = snapshot.inbox.filter((item) => selectedIds.includes(item.id));
      ...
      <section className="workspace-dialog inbox-directory-dialog" role="dialog" aria-modal="true" aria-labelledby="inbox-directory-title">
        ...
        {!selection ? (
          ...
          <div className="directory-consent">
            ...
            <p>{snapshot.account.status === 'connected' ? '这项授权绑定当前 Codex 账户、所选目录和成员；停用目录会同时撤回。' : '当前未连接 Codex，因此只能先启用本地导入。连接后可重新授权自动处理。'}</p>
...
export default function App() {
  ...
  const content = (() => {
    switch (page) {
      ...
      }} onImport={() => void handleImport()} onDropFiles={(files, personId) => void handleDroppedFiles(files, personId)} onAssign={handleBatchAssign} onIgnore={handleBatchIgnore} onOpenEvidence={(next) => void handleOpenEvidence(next)} onDirectories={() => {
