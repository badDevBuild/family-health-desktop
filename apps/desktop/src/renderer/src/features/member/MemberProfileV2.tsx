...
const adoptedActionStatusLabels: Record<LifestylePlanV2['adoptedActions'][number]['status'], string> = {
  proposed: '待安排', discussed: '已讨论', planned: '已计划', in_progress: '进行中',
  paused: '已暂停', completed: '已完成', dismissed: '已取消'
};
...
function SystemAnalysisCard({ detail, onOpenEvidence }: { detail: BodySystemDetailV2; onOpenEvidence(request: EvidenceRequest): void }) {
  ...
  return <article className="system-analysis-card">
    ...
    {!isCurrent && <div className="info-callout compact"><ShieldCheck size={18} /><div><strong>旧版解读，仅供参考</strong><p>这段说明尚未按当前成员级规则重新综合。原始事实仍可查看；请按标注日期理解旧结论。</p></div></div>}
...
function SystemDetail({ detail, assessment, search, onSearch, selectedMetric, onSelectMetric, onOpenEvidence }: {
  ...
  return <section className="system-detail">
    <div className="system-detail__hero"><span className={`system-directory__symbol is-${detail.summary.status}`}>{systemSymbol[detail.registry.id]}</span><div><span className="eyebrow">{detail.registry.name}</span><h2>{detail.registry.shortName}</h2><p>{detail.analysis?.status === 'stale' ? '旧版说明供回看；下面的指标和原始依据仍可查看。' : detail.analysis ? '先看结论和下一步，需要时再展开指标与原始依据。' : detail.summary.summary}</p></div></div>
...
function LifestyleProposalCard({ proposal, index, busy, canAdopt, onAdopt, onDecide, onOpenEvidence }: {
  ...
  return <article className={proposal.status === 'dismissed' ? 'is-muted' : ''}>
    ...
    <div>
      ...
      {!canAdopt && proposal.status === 'proposed' && <div className="info-callout compact"><ShieldCheck size={18} /><div><strong>这条旧版建议尚待复核</strong><p>新版综合完成前不能把它加入行动；以前已经采纳的行动及进度保持不变。</p></div></div>}
      <details>
      ...
      <div className="proposal-actions">
        ...
        {proposal.status === 'dismissed' && canAdopt && <button className="text-button" disabled={busy} onClick={() => onDecide('restore')}>恢复为待决定</button>}
      </div>
    </div>
  </article>;
}
...
export function MemberProfileV2({ snapshot, person, onSelectPerson, onOpenEvidence, onAddPerson, onEditPerson, onArchivedPeople, onAddNote, onExport, onImport, onRefreshAssessment, onExcludeDocument, onReincludeDocument, onDeleteDocument, onDeletedDocuments }: {
  ...
  return <div className="page-stack member-profile-v2">
    ...
    {!loading && overview?.personId === person.id && !loadError && overview && tab === 'overview' && <div className="member-v2-overview">
      <section className="panel member-health-headline"><div><span className="eyebrow">现在怎样</span><h2>{overview.headline}</h2><p>{overview.overview}</p>{!assessment && systems.some((system) => system.analysisStatus === 'stale') && <p>旧版解读可在“身体与指标”中回看，尚不能当作当前综合结论。</p>}<small>资料截至 {overview.latestClinicalDate ?? '日期待确认'}</small></div></section>
    ...
    {!loading && tab === 'guidance' && assessment && <div className="guide-grid">
      ...
      <section className="panel"><div className="panel__heading"><h3>已采纳行动</h3><StatusBadge tone="neutral">{plan?.adoptedActions.length ?? 0} 项</StatusBadge></div>{plan?.adoptedActions.length ? plan.adoptedActions.map((action) => <div className="completed-item adopted-action-card" key={action.id}><Check size={18} /><div><strong>{action.title}</strong>{action.userGoal.trim() !== action.title.trim() && <span>{action.userGoal}</span>}<p className="adopted-action-card__detail">{action.selectedStartingOption}</p><small>进度：{adoptedActionStatusLabels[action.status]}</small><small>负责人：{action.owner}{action.plannedTime ? ` · ${action.plannedTime}` : ''}</small>{action.progressNote && <small>备注：{action.progressNote}</small>}</div></div>) : <p className="muted-copy">还没有主动采纳的行动。</p>}</section>
    </div>}
    {!loading && tab === 'guidance' && !assessment && <div className="guide-grid">{plan && plan.proposals.length > 0 ? <>
      <section className="panel guide-hero"><span className="eyebrow">{plan.status === 'stale' ? '历史建议' : '本周重点'}</span><h2>{plan.status === 'stale' ? '此前生成的生活建议' : '一份成员级生活方案'}</h2><p>{plan.status === 'stale' ? '这些建议保留供回看；当前行动请以下方本人已采纳的记录为准。' : '相同方向已跨身体系统合并；建议不会自动变成你的计划，只有主动采纳后才进入行动层。'}</p>{plan.status === 'stale' && <div className="info-callout compact" role="status"><ShieldCheck size={18} /><div><strong>旧版建议已恢复展示</strong><p>内容仍可以阅读，但尚未按当前的来源和安全规则重新复核，所以暂时不能直接采纳。</p></div></div>}<div className="member-proposal-list">{plan.proposals.map((proposal, index) => <LifestyleProposalCard key={proposal.id} proposal={proposal} index={index} busy={adoptingProposalId !== null || decidingProposalId !== null} canAdopt={plan.status === 'current'} onAdopt={(adoption) => adoptProposal(proposal, adoption)} onDecide={(decision) => decideProposal(proposal.id, decision)} onOpenEvidence={onOpenEvidence} />)}</div></section>
      <section className="panel"><div className="panel__heading"><h3>已采纳行动</h3><StatusBadge tone="neutral">{plan.adoptedActions.length} 项</StatusBadge></div>{plan.adoptedActions.length > 0 ? plan.adoptedActions.map((action) => <div className="completed-item adopted-action-card" key={action.id}><Check size={18} /><div><strong>{action.title}</strong>{action.userGoal.trim() !== action.title.trim() && <span>{action.userGoal}</span>}<p className="adopted-action-card__detail">{action.selectedStartingOption}</p><small>进度：{adoptedActionStatusLabels[action.status]}</small><small>负责人：{action.owner}{action.plannedTime ? ` · ${action.plannedTime}` : ''}{action.dueDate ? ` · ${action.dueDate} 回顾` : ''}</small>{action.progressNote && <small>备注：{action.progressNote}</small>}</div></div>) : <p className="muted-copy">还没有主动采纳的行动。</p>}</section>
